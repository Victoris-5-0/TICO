...
6. Accounts are mandatory and there is no anonymous or guardian-approval flow in MVP. The original Supabase Auth implementation is superseded by [ADR 0003](0003-google-sign-in.md): Better Auth now provides Google-only account entry and PostgreSQL-backed sessions.
7. Next.js 16 on Vercel owns UI/game behavior and Prisma owns every table and migration in the shared PostgreSQL `public` schema. Pyodide in a browser worker executes code. FastAPI on AWS EC2 (portable container) owns AI behavior and uses synchronous SQLAlchemy mappings without migration authority. Supabase hosts PostgreSQL/storage; Better Auth runs in Next.js.
