...
Follow [layered bakery production](13-bakery-layered-production.md) and its [exact generation prompt log](bakery-v2-prompts.json) for the side-on visual preview. The fixed environment must contain no people or bread. Oven, awning, counter front/worktop, trays, loaves, tools, bags and actors are independent raster layers. Masters: `client/assets/source/bakery-v2/`. Runtime: `client/public/assets/bakery-v2/`. **These are no longer client-only.** `content/worlds/el_forn.yaml` now names the bakery-v2 layers directly: `visual.backdrop` is `bakery-v2/environment.webp`, the fifteen fixtures are its `sprites`, the ten drawable actors are its `characters`, and its `animations` are the phases `simulation.ts` actually runs. Adding or renaming a bakery-v2 asset is therefore a shared contract change — update the manifest in the same commit, and `ai-backend/scripts/check_manifests.py` will fail if a sprite points at a file that does not exist.
