...
The delivered set contains 23 runtime WebPs (approximately 1.2 MB): the environment, six fixtures, six props, and ten actor atlases. The 66 normalized individual actor frames stay in `client/assets/source/bakery-v2/frames/` for review, not in the public runtime directory. Small steam paths are native SVG; their opacity and the oven glow share the simulation clock. Regenerate optimized assets from the repository masters with `pnpm assets:bakery` inside `client/` (requires ImageMagick). No image-generation API call is needed to rebuild them.

## Scene graph and behavior

One fixed SVG coordinate system, 1600×900, scales the entire scene together. Depth order: environment → oven/effects → Hassan and carried tools → counter front → work surface/tray/stock → customers and handoffs → foreground plants. No percent-positioned actors or independently measured CSS coordinates.
