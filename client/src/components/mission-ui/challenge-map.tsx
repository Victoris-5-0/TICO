      ...
      <div className={styles.scene}>
        ...
        <ol className={styles.nodes}>
          {stage.nodes.map((node, index) => <li key={node.id} className={styles.nodePosition} style={{ left: `${node.x / 14.4}%`, top: `${node.y / 19.29}%` }}>
            <motion.button type="button" className={styles.node} data-status={node.status} aria-label={`${index + 1}. ${node.label} — ${statusLabels[locale][node.status]}`} aria-disabled={node.status === "locked"} aria-current={node.status === "current" ? "step" : undefined} onClick={() => { if (node.status !== "locked") onSelect(node, stage); }} whileHover={reduced || node.status === "locked" ? undefined : { y: -3 }} whileTap={reduced || node.status === "locked" ? undefined : { scale: 0.98 }}>
