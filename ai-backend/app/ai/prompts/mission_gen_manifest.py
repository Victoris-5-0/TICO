"""Mission generation prompt for the props-and-verbs manifest generator.

Was `app/ai/prompts/mission_gen.py` on `ai/foundations`. It moved aside because the
six-phase generator took that name: `app/ai/chains/mission_gen.py` composes ENCOUNTER
through ADAPT_REMIX from a world vocabulary, while this one asks for a single scenario
built from a manifest of allowed props and verbs. Both are called "mission generation",
and they are not the same thing.

Its only caller is `app/ai/graphs/mission_gen.py`. See the LEGACY section of
`app/ai/guards.py` for why that generator is still here.
"""
