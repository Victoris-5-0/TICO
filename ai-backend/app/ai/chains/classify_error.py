...
def classify_error(
    *,
    ...
    return AnalyzeResponse(
        # `error_family`/`error_tag`, not `family`/`tag`: the names come from
        # `app/schemas/submissions.py`, which is what `/openapi.json` publishes and what
        # `client/src/lib/ai/types.ts` is generated from.
